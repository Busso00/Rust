...
impl FileSystem {
    ...
    fn search<'a>(&'a mut self,queries: &[&'a str]) -> Option<MatchResult<'a>>{
        ...
        for query in queries{
            ...
            let mut unvisited_dir = vec![Rc::new(RefCell::new(self.root.clone()))];
			//parsing the query
...
fn main() {
    ...
    println!("{:?}",res);
    println!("test name=prova5");
    let args=["name:prova5"];
    let res=root.search(&args);
    println!("{:?}",res);
    println!("test content=ABCDEFGHI");
    let args=["content:ABCDEFGHI","older:999"];
    let res=root.search(&args);
    println!("{:?}",res);
    println!("test name:nf2111 older:2000");
    let args = ["name:nf2111","name:nf3"];
    let res=root.search(&args);
    println!("{:?}",res);
}
